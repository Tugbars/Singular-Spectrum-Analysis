...
int main()
{
    ...
    // Compare singular values (only first 10 - later ones may be small/degenerate)
    printf("\nSingular value comparison (seq vs block):\n");
    double max_sigma_diff = 0;
    int n_check = (k < 10) ? k : 10; // Only check dominant components
    for (int i = 0; i < k; i++)
    {
        double diff = fabs(ssa_seq.sigma[i] - ssa_block.sigma[i]);
        double rel_diff = diff / (ssa_seq.sigma[i] + 1e-15);
        if (i < n_check && rel_diff > max_sigma_diff)
            max_sigma_diff = rel_diff;
        if (i < 5)
        {
    ...
    printf("Max relative sigma difference (top %d): %.2e\n", n_check, max_sigma_diff);
    ...
    {
        printf("[WARN] Top-10 singular value relative difference > 1%% (%.2e)\n", max_sigma_diff);
        // Not a hard fail - reconstruction is what matters
    }
