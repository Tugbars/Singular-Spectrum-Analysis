...
int main(int argc, char** argv) {
    ...
    int ret = ssa_mkl_init(&ssa, x, N, L);
    double t1 = get_time_ms();
    
    if (ret != 0) {
        fprintf(stderr, "ERROR: ssa_mkl_init failed\n");
        free(x);
        return 1;
    }
    ...
    ret = ssa_mkl_decompose(&ssa, k, 150);
    double t3 = get_time_ms();
    
    if (ret != 0) {
        fprintf(stderr, "ERROR: ssa_mkl_decompose failed\n");
        ssa_mkl_free(&ssa);
        free(x);
        return 1;
    }
    ...
    for (int i = 0; i < k && i < 10; i++) {
        double var = ssa_mkl_variance_explained(&ssa, i, i) * 100.0;
        cumulative += var;
