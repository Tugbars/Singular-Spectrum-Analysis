...
static inline void ssa_opt_reverse(const double* in, double* out, int n) {
    ...
    for (int i = 0; i < n; i += 4) {
        ...
        if (i+2 < n) out[i+2] = src[-i-2];
        if (i+3 < n) out[i+3] = src[-i-3];
    }
}
...
static inline double ssa_opt_normalize(double* v, int n) {
    ...
    if (norm > 1e-15) ssa_opt_scal(v, n, 1.0 / norm);
    return norm;
}
...
static void ssa_opt_fft_r2c(SSA_Opt* ssa, const double* input, int input_len, double* output) {
    int n = ssa->fft_len;
    ssa_opt_zero(output, 2 * n);
    for (int i = 0; i < input_len; i++) output[2*i] = input[i];
...
static void ssa_opt_fft_c2r(SSA_Opt* ssa, double* data, double* output, int output_len) {
    ...
    for (int i = 0; i < output_len; i++) output[i] = data[2*i];
}
...
int ssa_opt_decompose(SSA_Opt* ssa, int k, int max_iter) {
    ...
    for (int comp = 0; comp < k; comp++) {
        ...
        for (int iter = 0; iter < max_iter; iter++) {
            ...
            if (sqrt(diff) < SSA_CONVERGENCE_TOL && iter > 10) break;
        }
...
int ssa_opt_decompose_block(SSA_Opt* ssa, int k, int block_size, int max_iter) {
    ...
    while (comp < k) {
        ...
        // QR factorization to orthonormalize V_block
...
int ssa_opt_reconstruct(const SSA_Opt* ssa, const int* group, int n_group, double* output) {
    ...
    for (int g = 0; g < n_group; g++) {
        ...
        for (int i = 0; i < L; i++) ws1[2*i] = sigma * u_vec[i];
        ssa_opt_fft_builtin(ws1, fft_n, -1);
        
        // FFT of v
        ssa_opt_zero(ws2, 2 * fft_n);
        for (int i = 0; i < K; i++) ws2[2*i] = v_vec[i];
...
#ifdef SSA_USE_MKL
    if (ssa->fft_handle) DftiFreeDescriptor(&ssa->fft_handle);
    if (ssa->fft_batch_handle) DftiFreeDescriptor(&ssa->fft_batch_handle);
    if (ssa->rng) vslDeleteStream(&ssa->rng);
...
int ssa_opt_get_noise(const SSA_Opt* ssa, int noise_start, double* output) {
    if (!ssa || !ssa->decomposed || noise_start < 0) return -1;
    ...
    for (int i = 0; i < n_noise; i++) group[i] = noise_start + i;
...
double ssa_opt_variance_explained(const SSA_Opt* ssa, int start, int end) {
    ...
    if (end < 0 || end >= ssa->n_components) end = ssa->n_components - 1;
    
    double sum = 0;
    for (int i = start; i <= end; i++) sum += ssa->eigenvalues[i];
